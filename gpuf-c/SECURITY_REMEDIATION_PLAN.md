...
本轮已经把 P0/P1 从计划项推进到代码、脚本、CI 和文档层面的可验收实现。P2/P3 中依赖 Android/iOS 真机、控制连接 TLS 协议迁移、真实 release artifact 签名的部分仍保留为 release/mobile gate；不能在当前 Linux 工作区伪造为已完成。
...
| P2-3/P2-4/P2-6 | 部分完成 | `static mut` 源码 grep 清零；全局状态改锁/原子；TURN/P2P secret redacted/zeroize；`SecurityConfig` 和高危配置告警已落地。`worker_sdk`、JNI、Android SDK 和 `src/lib.rs` 首批 llama.cpp FFI wrapper 路径继续移除不必要 `unsafe`，并为 chat-template FFI、C 字符串、callback `user_data`、token buffer、sampler/context/model helper 调用补充 `SAFETY` 说明；Linux `cargo check` 和 Android arm64 target check 均通过。`src/lib.rs` multimodal/generation 后半段 unsafe block 逐项说明仍需继续收敛。 |
| P2-5 | TLS opt-in、证书回归和移动 additive TLS transport 已落地，平台握手证据仍为 release gate | `gpuf-s --control-tls` 与 `gpuf-c --control-tls --control-tls-server-name ...` 已实现；本地明文兼容、TLS 握手、同 CA 轮换证书、过期证书拒绝测试通过。移动 C/JNI 新增 `start_remote_worker_with_tls` / `startRemoteWorkerWithTls`，旧入口保持明文兼容；Linux `cargo check`、mobile TLS policy/config 单测和 `aarch64-linux-android` target compile 通过。当前 Linux 主机无 `xcodebuild`/`xcrun` 且 `adb devices` 无连接设备；iOS target compile、Android/iOS instrumentation/simulator/device TLS/pinning handshake 证据仍归入移动 release gate。 |
...
| P3-2/P3-3 | 部分完成/release gate | SBOM/签名已有模板和证据脚本；Android SDK archive `target/gpufabric-android-sdk-v9.0.0.tar.gz` 已生成并通过 artifact-required SHA256 evidence gate。证据脚本已支持 `GPUF_REQUIRE_ARTIFACTS=1` / `GPUF_REQUIRE_SIGNING=1` 严格模式，缺签名证明会失败。当前 Linux 主机未安装 `cosign`/`minisign`，`gpg` 无 secret key；真实签名仍需 release job 使用正式 key 生成。standalone API 已有 opt-in content safety 和 metrics 测试，插件化/全链路策略仍为后续治理。 |

SDK 兼容性结论：当前改动保持 `gpuf-c` 既有公开 C/JNI/移动 SDK 函数名和参数签名兼容，未删除或重命名旧 public header/API。新增 TLS 能力采用 additive API：C `start_remote_worker_with_tls(...)` 与 JNI `RemoteWorker.startRemoteWorkerWithTls(...)`；旧 `start_remote_worker(...)` / `startRemoteWorker(...)` 仍保持明文兼容。行为层面更严格：远程 worker 不再回退到硬编码公网地址，集成方必须显式传入 `serverAddr`/端口；远程模型和 SDK 发布包必须使用 SHA256 校验；公开监听、跳过校验、明文控制连接等高风险路径必须显式配置并进入日志/metrics。CLI/config 方式的 `gpuf-c` 已支持控制连接 TLS opt-in；移动 TLS/pinning 真机/模拟器证据仍作为 release gate 单独闭环。
