...
| P2-3/P2-4/P2-6 | 部分完成 | `static mut` 源码 grep 清零；全局状态改锁/原子；TURN/P2P secret redacted/zeroize；`SecurityConfig` 和高危配置告警已落地。`worker_sdk`、JNI 和 Android SDK 路径继续移除不必要 `unsafe`，并为 chat-template FFI、C 字符串、callback `user_data`、llama.cpp helper 调用补充 `SAFETY` 说明；`cargo check` Linux 与 Android arm64 target 均通过。`src/lib.rs` 内完整 FFI unsafe block 逐项说明仍需继续收敛。 |
