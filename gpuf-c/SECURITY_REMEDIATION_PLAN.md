...
本轮已经把 P0/P1 从计划项推进到代码、脚本、CI 和文档层面的可验收实现。2026-06-09 已在拉取最新代码后完成 Android SDK 重新构建、SHA256 校验、Android arm64 target check 和 ARM64 Android 10 真机推理脚本验证。P2/P3 中依赖 iOS/macOS、Android/iOS TLS/pinning/instrumentation/sanitizer、Android Keystore/iOS Keychain 审计、真实 release artifact 签名的部分仍保留为 release/mobile gate；不能在当前 Linux 工作区伪造为已完成。
...
| P2-1/P2-2 | 部分完成，平台证据仍为强制 release gate | C/JNI 已新增 `start_remote_worker_with_tls` / `startRemoteWorkerWithTls`，在旧协议外包 TLS，并支持 CA bundle、SNI/server name、SHA256 leaf pin；旧 `start_remote_worker` / `startRemoteWorker` 保持明文兼容。Android arm64 target compile 和 packaged Android SDK 真机推理脚本已在本地 NDK 25.1.8937393 下通过；脚本覆盖 `gpuf_init`、`gpuf_load_model`、`gpuf_create_context`、`gpuf_generate_final_solution_text`、`gpuf_generate_with_sampling`。仍需要 Android Keystore/iOS Keychain、权限审计、Android instrumentation、iOS target/simulator/device、ASAN/TSAN 和 Android/iOS TLS/pinning 握手证据；`scripts/mobile_sdk_release_gate.sh` 已支持 `GPUF_REQUIRE_MOBILE_EVIDENCE=1`，正式移动 SDK release 缺平台证据会失败。 |
...
| P2-5 | TLS opt-in、证书回归和移动 additive TLS transport 已落地，平台握手证据仍为 release gate | `gpuf-s --control-tls` 与 `gpuf-c --control-tls --control-tls-server-name ...` 已实现；本地明文兼容、TLS 握手、同 CA 轮换证书、过期证书拒绝测试通过。移动 C/JNI 新增 `start_remote_worker_with_tls` / `startRemoteWorkerWithTls`，旧入口保持明文兼容；Linux `cargo check`、mobile TLS policy/config 单测、`aarch64-linux-android` target compile 和 Android 真机本地推理脚本通过。当前 Linux 主机无 `xcodebuild`/`xcrun`；iOS target compile、Android/iOS instrumentation/simulator/device TLS/pinning handshake 证据仍归入移动 release gate。 |
...
SDK 兼容性结论：当前改动保持 `gpuf-c` 既有公开 C/JNI/移动 SDK 函数名和参数签名兼容，未删除或重命名旧 public header/API。Android SDK 重新构建和真机测试修复只改变构建缓存选择、NDK ABI 校验和测试脚本调用方式，不要求旧集成方改 Java/C/REST 调用签名。新增 TLS 能力采用 additive API：C `start_remote_worker_with_tls(...)` 与 JNI `RemoteWorker.startRemoteWorkerWithTls(...)`；旧 `start_remote_worker(...)` / `startRemoteWorker(...)` 仍保持明文兼容。行为层面更严格：远程 worker 不再回退到硬编码公网地址，集成方必须显式传入 `serverAddr`/端口；远程模型和 SDK 发布包必须使用 SHA256 校验；公开监听、跳过校验、明文控制连接等高风险路径必须显式配置并进入日志/metrics。CLI/config 方式的 `gpuf-c` 已支持控制连接 TLS opt-in；移动 TLS/pinning 真机/模拟器证据仍作为 release gate 单独闭环。
