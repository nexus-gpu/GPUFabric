...
impl ClientWorker {
    ...
    async fn turn_connect_peer(
        ...
        let txid = Self::stun_new_txid();
        let addr_val = Self::turn_encode_xor_peer_address(peer, &txid);
                                        ...
                                        .await
                                    {
                                        Ok(Ok(stream)) => {
                                            ...
                                            #[cfg(not(target_os = "android"))]
                                            {
                                                ...
                                                tokio::spawn(async move {
                                                    if let Err(e) =
                                                        Self::serve_p2p_stream_with_engine(
                                                            engine,
                                                            stream,
                                                            connection_id,
                                                        )
