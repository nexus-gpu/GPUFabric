...
use ash::{vk, Entry};
#[cfg(feature = "vulkan")]
use common::{DevicesInfo, EngineType, OsType};
#[cfg(feature = "vulkan")]
use sysinfo;
...
pub async fn collect_device_info_vulkan_cross_platform() -> Result<(DevicesInfo, u16)> {
    ...
    debug_println!(
        "Starting Vulkan API device info collection for {}...",
        platform_name
    );
    ...
    for (index, &physical_device) in physical_devices.iter().enumerate() {
        ...
        debug_println!(
            "  Queue Count: Graphics{} Compute{} Transfer{}",
            graphics_queues,
            compute_queues,
            transfer_queues
        );
        ...
        debug_println!(
            "    - Shader Storage: {}",
            features.shader_storage_image_extended_formats != 0
        );

        // Store GPU details for summary
        if !is_software_or_cpu {
            gpu_details.push(format!(
                "GPU{}: {} ({}GB, {}TFLOPS)",
                index + 1,
                device_name,
                device_memory_gb,
    ...
    debug_println!(
        "  System Memory Usage: {}% ({}GB/{}GB)",
        ((used_memory as f32 / total_memory as f32) * 100.0) as u64,
...
fn estimate_gpu_tflops_cross_platform(vendor_id: u32, _device_id: u32, device_name: &[u8]) -> u16 {
    ...
    match vendor_id {
        // ARM Mali GPUs
        0x13B5 => {
            if name_str.contains("G715") || name_str.contains("G710") {
                5
            } else if name_str.contains("G68") || name_str.contains("G57") {
                3
            } else if name_str.contains("G52") || name_str.contains("G31") {
                1
            } else {
                2
            }
        ...
        0x5143 => {
            if name_str.contains("740") || name_str.contains("730") {
                6
            } else if name_str.contains("660") || name_str.contains("650") {
                4
            } else if name_str.contains("640") || name_str.contains("630") {
                2
            } else {
                3
            }
        ...
        // PowerVR GPUs
        0x1010 => {
            if name_str.contains("BXS") {
                4
            } else if name_str.contains("XE") {
                2
            } else {
                1
            }
        ...
        // NVIDIA GPUs
        0x10DE => {
            if name_str.contains("RTX 4090") {
                83
            } else if name_str.contains("RTX 4080") {
                48
            } else if name_str.contains("RTX 4070") {
                29
            } else if name_str.contains("RTX 4060") {
                16
            } else if name_str.contains("RTX 3090") {
                36
            } else if name_str.contains("RTX 3080") {
                30
            } else if name_str.contains("RTX 3070") {
                20
            } else if name_str.contains("RTX 3060") {
                13
            } else if name_str.contains("Tegra") {
                8
            } else {
                10
            }
        ...
        // AMD GPUs
        0x1002 => {
            if name_str.contains("RX 7900") {
                52
            } else if name_str.contains("RX 7800") {
                28
            } else if name_str.contains("RX 7700") {
                20
            } else if name_str.contains("RX 6900") {
                23
            } else if name_str.contains("RX 6800") {
                16
            } else if name_str.contains("RX 6700") {
                11
            } else {
                8
            }
        ...
        // Intel GPUs
        0x8086 => {
            if name_str.contains("Arc A770") {
                16
            } else if name_str.contains("Arc A750") {
                12
            } else if name_str.contains("Arc A380") {
                8
            } else if name_str.contains("Iris Xe") {
                2
            } else {
                1
            }
...
#[allow(dead_code)] // This function is used but may appear unused due to conditional compilation
fn estimate_gpu_metrics_fallback(
    _device_count: u16,
    _total_tflops: u16,
    _total_memory_gb: u32,
) -> (u64, u64, u64, u64) {
...
fn try_sysfs_gpu_metrics(
    ...
    let device_dir = candidates
        .iter()
        .find(|(_, v)| {
            wanted_vendor.is_some() && v.is_some() && v.unwrap() == wanted_vendor.unwrap()
        })
        .map(|(p, _)| p.clone())
    ...
    let mem_usage = {
        let total = read_trimmed(&device_dir.join("mem_info_vram_total"))
            .ok()
            .and_then(|s| parse_u64(&s));
        let used = read_trimmed(&device_dir.join("mem_info_vram_used"))
            .ok()
            .and_then(|s| parse_u64(&s));
        match (total, used) {
    ...
    let (power_usage, temp) = {
        ...
        if let Ok(hwmons) = std::fs::read_dir(&hwmon_base) {
            for h in hwmons.flatten() {
                let hpath = h.path();
                let power_paths = [hpath.join("power1_average"), hpath.join("power1_input")];
