...
#define LOG_TAG "GPUFabric-Android-Direct-NDK"
...
Java_com_gpuf_c_GPUEngine_init(JNIEnv *env, jobject thiz) {
    ...
    LOGI("Initializing GPUFabric Android Direct NDK SDK");
    ...
    if (result == 0) {
        LOGI("GPUFabric initialized successfully in %ld ms", duration.count());
    } else {
        LOGE("GPUFabric initialization failed: %s", gpuf_get_last_error());
    }
...
Java_com_gpuf_c_GPUEngine_generate(JNIEnv *env, jobject thiz, jstring prompt) {
    ...
    LOGD("Generating response for prompt: %.100s...", prompt_str.c_str());
    ...
    LOGD("Generation completed in %ld ms", duration.count());
...
Java_com_gpuf_c_GPUEngine_loadModel(JNIEnv *env, jobject thiz, jstring model_path) {
    ...
    if (result == 0) {
        LOGI("Model loaded successfully in %ld ms", duration.count());
    } else {
        LOGE("Model loading failed: %s", gpuf_get_last_error());
    }
...
Java_com_gpuf_c_GPUEngine_cleanup(JNIEnv *env, jobject thiz) {
    ...
    LOGI("Cleaning up GPUFabric Android Direct NDK SDK");
    
    gpuf_cleanup();
    g_jni_initialized = false;
    
    LOGI("GPUFabric cleaned up successfully");
}
...
Java_com_gpuf_c_GPUEngine_batchGenerate(JNIEnv *env, jobject thiz, jobjectArray prompts) {
    ...
    LOGD("Batch generating %d responses", size);
    ...
    LOGD("Batch generation completed");
    return result;
}
