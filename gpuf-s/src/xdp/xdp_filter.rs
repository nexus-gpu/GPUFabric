...
impl XdpFilter {
    ...
    pub async fn add_api_key(&self, key: &[u8]) -> Result<()> {
        ...
        let mut api_keys: HashMap<_, [u8; 16], u8> =
            HashMap::try_from(bpf.map_mut("api_keys").ok_or(anyhow::anyhow!("Failed to get map"))?)?;
    ...
    pub async fn remove_api_key(&self, key: &[u8]) -> Result<()> {
        ...
        println!("Removed API key: {:?}", String::from_utf8_lossy(&padded_key));
        Ok(())
    }
}
...
async fn test_xdp_filter() {
    let xdp_filter = XdpFilter::new("enp5s0", "./gpuf-s/src/xdp/xdp_auth_filter.o").await.expect("Failed to create XDP filter");
    xdp_filter.add_api_key(b"1234567890").await.expect("Failed to add API key");
    xdp_filter.remove_api_key(b"1234567890").await.expect("Failed to remove API key");
}
